...
1. `run.command` 로 앱 실행
2. 주소창 오른쪽 끝 **설치 아이콘 (⊕)** 클릭
3. "설치" 확인 → Dock에 **스킬셋** 앱 추가됨

> 설치 아이콘이 보이지 않으면: 주소창 오른쪽 `⋮` (더보기) → **"UX AI 스킬셋 설치…"**

### Mac — Safari

1. `run.command` 로 앱 실행, Safari에서 `http://localhost:8080/ux_skillset_v10.html` 접속
2. 상단 **공유 버튼 (□↑)** 클릭
3. **"Dock에 추가"** 선택 → 이름 확인 후 추가

### Windows — Edge (권장)

1. `run.bat` 으로 앱 실행
2. 주소창 오른쪽 **설치 아이콘 (⊕)** 클릭
3. "설치" 확인 → 작업 표시줄 및 시작 메뉴에 추가됨

### Windows — Chrome

1. `run.bat` 으로 앱 실행
2. 주소창 오른쪽 `⋮` → **"UX AI 스킬셋 설치…"** 클릭
3. "설치" 확인
