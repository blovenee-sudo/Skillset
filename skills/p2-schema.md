...
## Step 5 — 요약 리포트 HTML 출력

설계 요약 바로 아래에, 핵심 결과를 1페이지로 요약한 HTML을 코드블록(` ```html `)으로 출력합니다.

- **헤더**: 프로젝트명 · Phase 2 · 작성일
- **본문**: 엔티티 목록 · 사용자 플로우 요약 · Constraints · 미결 항목 (각 항목을 카드/섹션으로 구분)
- **하단**: 「인쇄 / PDF 저장」 버튼 (`onclick="window.print()"`) · 저장 파일명 안내 `[프로젝트명]-schema-report.html`
