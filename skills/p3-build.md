...
## Step 4 — 요약 리포트 HTML 출력

프로토타입 안내 바로 아래에, 핵심 결과를 1페이지로 요약한 HTML을 코드블록(` ```html `)으로 출력합니다.

- **헤더**: 프로젝트명 · Phase 3 · 작성일
- **본문**: 구현 화면 목록 · 핵심 플로우 · Constraints 반영 여부 · 미구현 항목 (각 항목을 카드/섹션으로 구분)
- **하단**: 「인쇄 / PDF 저장」 버튼 (`onclick="window.print()"`) · 저장 파일명 안내 `[프로젝트명]-build-report.html`
- 외부 의존성 없음 · 인라인 CSS · 시스템 폰트 · `@media print` 로 버튼 숨김 처리
