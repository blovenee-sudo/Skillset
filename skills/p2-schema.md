...
## Step 2 — JSON Schema 설계

아래 구조로 JSON 스키마를 설계합니다. **JSON 코드 출력은 하지 않습니다** — Step 3 시각화를 먼저 출력하고, Step 5에서 JSON을 출력합니다.
...
**출력 순서: 3-F (서비스 구조) → 3-A (ER 다이어그램) → 3-B (필드 테이블) → 3-C (사용자 플로우) → 3-D (기능 정의서) → 3-E (엣지케이스)**
...
각 엔티티를 Markdown 테이블로 **코드블록 없이 직접** 출력합니다. `entities` 배열 순서대로, 각각 `### EntityName` 헤더 아래에 작성합니다.

출력 형식 (코드블록 없이 이 형태 그대로):
...
다이어그램 아래에 해당 플로우의 step을 **코드블록 없이** Markdown 테이블로 출력합니다.
...
`features` 배열을 아래 형식의 Markdown 테이블로 **코드블록 없이 직접** 출력합니다.
...
`edge_cases` 배열을 아래 형식의 Markdown 테이블로 **코드블록 없이 직접** 출력합니다.
...
> **✅ Phase 2 완료**
>
> **① 저장** — 아래 Step 5 JSON을 복사해 스킬셋 앱 해당 프로젝트의 **Phase 2 산출물 슬롯**에 붙여넣어 저장하세요.
> **② 검토** — `/qa` 스킬에 Step 5 JSON을 붙여넣어 QA 리포트를 받고 체크리스트를 검토하세요.
> **③ 다음 단계** — 검토 완료 후 `/build` 스킬에 Step 5 JSON을 붙여넣으면 HTML 프로토타입을 생성합니다.
```
...
## Step 5 — Phase 3 저장용 JSON

Step 2에서 설계한 JSON 스키마를 코드블록으로 출력합니다.

> **Phase 3 저장용 JSON** — 아래 JSON을 복사해 스킬셋 앱 Phase 2 산출물 슬롯에 저장하거나, `/build`에 붙여넣으세요.

Step 2의 JSON 구조 전체를 `\`\`\`json ... \`\`\`` 코드블록으로 출력합니다.
